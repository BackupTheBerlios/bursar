...
/**
  * Constructor
  */
AccountTreeModel::AccountTreeModel(BursarDb *db)
    : QStandardItemModel()
{
...
void AccountTreeModel::load()
{
    ...
    // load accounts
    curItem->appendRows(m_db->accountNames(ACT_CURRENT_ACCOUNT));
    curItem->appendColumn(m_db->accountBanks(ACT_CURRENT_ACCOUNT));
    curItem->appendColumn(m_db->accountCountries(ACT_CURRENT_ACCOUNT));
    curItem->appendColumn(m_db->accountBalances(ACT_CURRENT_ACCOUNT));

    savItem->appendRows(m_db->accountNames(ACT_SAVINGS_ACCOUNT));
    savItem->appendColumn(m_db->accountBanks(ACT_SAVINGS_ACCOUNT));
    savItem->appendColumn(m_db->accountCountries(ACT_SAVINGS_ACCOUNT));
    savItem->appendColumn(m_db->accountBalances(ACT_SAVINGS_ACCOUNT));

    loanItem->appendRows(m_db->accountNames(ACT_LOAN_ACCOUNT));
    loanItem->appendColumn(m_db->accountBanks(ACT_LOAN_ACCOUNT));
    loanItem->appendColumn(m_db->accountCountries(ACT_LOAN_ACCOUNT));
    loanItem->appendColumn(m_db->accountBalances(ACT_LOAN_ACCOUNT));

    cardItem->appendRows(m_db->accountNames(ACT_CREDIT_CARD));
    cardItem->appendColumn(m_db->accountBanks(ACT_CREDIT_CARD));
    cardItem->appendColumn(m_db->accountCountries(ACT_CREDIT_CARD));
    cardItem->appendColumn(m_db->accountBalances(ACT_CREDIT_CARD));
}
